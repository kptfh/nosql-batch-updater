...
import static nosql.batch.update.aerospike.basic.util.BasicFlakingOperationsUtil.flakingOperations;
...
public class BasicRecoveryTest extends RecoveryTest {
    ...
    static BatchOperations<AerospikeBasicBatchLocks, List<Record>, AerospikeLock, Value> operations
            = flakingOperations(client, reactorClient, clock, failsAcquire, failsUpdate, failsRelease, failsDeleteBatchInWal);
    ...
    @Override
    protected void runCompleter(){
        clock.setTime(STALE_BATCHES_THRESHOLD.toMillis() + 1);
        walCompleter.completeHangedTransactions();
    }
